            ...
            if (ImGui::BeginTabItem("Scene Eliments"))
            {
                ...
                if (ImGui::TreeNodeEx(ICON_FA_CUBE" Spidex Scean"))
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        if (i < CubeIndex) //#######
                        {
                            // ICON_FA_LANDMARK ICON_FA_MOON
                            // Items 0..2 are Tree Node ICON_FA_VIDEO ICON_FA_CUBE ICON_FA_SUN ICON_FA_CLOUD_MOON ICON_FA_BOX_OPEN
                            bool node_open = ImGui::TreeNodeEx((void*)(intptr_t)i, node_flags, ICON_FA_CUBE "Selectable Node %d", i);
        ...
        if (ImGui::BeginTabBar("##Main", ImGuiTabBarFlags_None))
        {
            if (ImGui::BeginTabItem("Property Lab"))
            {
               ...
               // I need to loop thought all images in the texture folder
                ...
                if (ImGui::CollapsingHeader(ICON_FA_SPINNER" Transform", ImGuiTreeNodeFlags_DefaultOpen))
                {
                    ...
                    ImGui::Text("Position:  ");
                    ImGui::SameLine();
                    ImGui::Button("Reset"); // Reset to 1,1,1
                    ImGui::SameLine();
                    ImGui::DragFloat3("##pos", pos_val, 1.0f, 1.0f, 1.0f);
                    ImGui::TableNextRow();
                    ImGui::TableNextColumn();
                    ImGui::Text("Rotation:  ");
                    ImGui::SameLine();
                    ...
                    ImGui::SameLine();
                    ImGui::DragFloat3("##Rot", rotation_val, 0.3f, 0.3f, 0.3f);
                   
                    ImGui::TableNextRow();
                    ImGui::TableNextColumn();
                    ImGui::Text("Scale:       ");
                    ImGui::SameLine();
                    ImGui::Button("Reset"); // 1,1,1
                    ImGui::SameLine();
                    ImGui::DragFloat3("##Scale", scale_val, 0.3f, 0.3f, 0.3f);
...
private:
    ...
    float rotation_val[3] = { 0.3f, 0.3f, 0.3f };
